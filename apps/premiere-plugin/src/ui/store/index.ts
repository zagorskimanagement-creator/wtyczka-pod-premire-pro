...
export interface AppState {
  ...
  uploadVideos: (files: File[], projectName: string, platform: string) => Promise<string>;
...
export interface ApplyEditResult {
  ...
  videos: Array<{ name: string; storagePath: string; durationSeconds: number | null }>;
  titleSuggestion: string | null;
  descriptionSuggestion: string | null;
  hashtags: unknown;
}
    ...
    (set, get) => ({
      ...
      uploadVideos: async (files, projectName, platform) => {
        ...
        const videos = await Promise.all(files.map(async (file, i) => {
          ...
          try {
            if (filePath) await tm.importVideoToProject(filePath);
          } catch { /* non-fatal */ }
          return {
            id: `video-${Date.now()}-${i}`,
            name: clipName,
      ...
      triggerAnalysis: async (projectId, options) => {
        ...
        const videoDescriptions = videoList.map((v, i) =>
          `  Video ${i} (clipIndex ${i}): "${v.name}" — ${Math.round((v.durationSeconds ?? 60))}s`,
        ).join('\n');
  ...
  "hashtags": ["#viral", "#fyp", "#trending"]
        ...
        try {
          ...
          const plan = JSON.parse(jsonStr) as {
            ...
            keepSegments?: Array<{ startMs: number; endMs: number; clipIndex?: number }>;
            zooms?: unknown[]; captions?: unknown[]; hashtags?: string[];
          };

          type Seg = { startMs: number; endMs: number; clipIndex?: number };
          const rawSegs: Seg[] = plan.keepSegments ?? [{ startMs: startGuess, endMs: startGuess + targetMs }];
          ...
          if (isMulti) {
            ...
            segs = videoList.map((v, i) => {
              ...
              if (end - start > share) end = start + share;
              if (durMs > 0 && end > durMs) { end = durMs; start = Math.max(0, end - share); }
